...
async def test_write_capability():
    ...
    logger.info(f"   👤 Usuario de prueba: {test_id}")

    # 1. Crear/Actualizar usuario con un calendar_id ficticio
    fake_email = "test.dummy@gmail.com"
    logger.info(f"   💾 Escribiendo calendar_id='{fake_email}'...")
    ...
    if not success:
        logger.error("   ❌ FALLO: No se pudo escribir (¿Está USE_FIRESTORE=true?)")
        return

    logger.info("   ✅ Escritura reportada como exitosa.")

    # 2. Leer inmediatamente para verificar consistencia
    logger.info("   📖 Leyendo usuario de vuelta...")
    user = await IdentityManager.get_user(test_id)

    logger.info(f"   🔍 Recuperado: Nombre='{user.name}', CalendarID='{user.calendar_id}'")

    if user.calendar_id == fake_email:
        logger.info("   🎉 SUCCESS: El dato ha persistido y se ha recuperado correctamente.")
    else:
        logger.error(f"   ❌ FAIL: Dato esperado '{fake_email}', recibido '{user.calendar_id}'")
...
if __name__ == "__main__":
    ...
    if not os.getenv("USE_FIRESTORE"):
        logger.warning("⚠️  ADVERTENCIA: USE_FIRESTORE no detectado en entorno. Puede fallar si no hay credenciales.")
